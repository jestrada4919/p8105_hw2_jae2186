...
Data  from FiveThirtyEight looking at political party and economics measures is uploaded in respective code chunks:
...
The second data set then provided dates and closing values (as `close`) for the Standard & Poor’s stock market index (S&P) with `r length(snp$year)` monthly values (taken on the 15th of each month) that started in `r snp$month[1]` `r min(snp$year)` and going until `r snp$month[length(snp$month)]` `r max(snp$year)`.

The third data set provided the monthly rates of umemployment, covering `r length(unemploy$year)` months of data, starting in `r unemploy$month[1]` `r min(unemploy$year)` and going until `r unemploy$month[length(unemploy$month)]` `r max(unemploy$year)`.
...
Similarly, the following code chunk results in a table providing by ethnicity the top names for each year from 2011 to 2016.
...
This last code chunk produces a scatter plot specifically looking at male, white non-Hispanic children born in 2016 in NYC and how the frequency of a name given to someone in this group compares to the name's rank. 
